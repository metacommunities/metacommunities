...
# ------------------------------------------------------------------------------
m <- dbDriver("MySQL")
con <- dbConnect(m, user=user_local, password=passwd_local, dbname=db_local)     
...
# ------------------------------------------------------------------------------
sql <- "ALTER TABLE tags ADD repo_count SMALLINT DEFAULT 0;"
dbGetQuery(con, sql)

for (i in 1:nrow(tag_repo)) {
    tg <- tag_repo$tag[i]
    rc <- tag_repo$repo_count[i]
    sql <- paste("UPDATE tags SET repo_count=", rc, " WHERE tag='", tg, "';", sep="")
    dbGetQuery(con, sql)
    cat(".")
}
