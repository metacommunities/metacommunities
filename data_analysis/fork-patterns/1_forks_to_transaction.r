...
rules = apriori(trans_actor_fork, parameter = list(support=0.002, confidence=0.2))
summary(rules)
inspect(rules)
...
fork_melted$repository_name = as.character(fork_melted$repository_name)
fork_melted$actor_attributes_login = as.character(fork_melted$actor_attributes_login)
fork_list = dlply(fork_melted, 'actor_attributes_login', function(x){x[,1]})
head(fork_list)
tail(fork_list)
length(fork_list)
trans_actor_fork = as(fork_list, 'transactions')
image(trans_actor_fork)
summary(trans_actor_fork)
itemFrequencyPlot(trans_actor_fork, support=0.007, cex.names=0.7)
