    /* Q_1 count events containing 'config, test, build' */
        ...
        where  regexp_match(payload_commit_msg,  'config|test|build')
        group by type
        order by event desc
    ...
    /* show many comments and commit messages mention 'config'?*/
        ...
        where payload_comment_body contains 'config' or payload_commit_msg contains 'config'
        group by repository_url, payload_comment_body, payload_commit_msg
