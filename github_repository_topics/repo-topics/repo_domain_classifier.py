...
## using scikit-learn implementation just to check here
clf = nb.BernoulliNB(alpha = 1.5)
Y_train = y[split].values
X_train = indicator_m[split,]

X_test = indicator_m[mask]
Y_test = y[mask].values
Y_pred = clf.predict(X_test)
print 'Accuracy is {}%'.format(100* sum(Y_pred == Y_test)/float(Y_test.shape[0]))
